...
public final class ObjectSizer {
	...
	public static void main(String[] aArguments) {
		String abc = "com.webex.webapp.MemoryBean";
		Class theClass = null;
		try {
			theClass = Class.forName(abc);
		} catch (Exception ex) {
			System.err.println("Cannot build a Class object: " + aArguments[0]);
			System.err
					.println("Use a package-qualified name, and check classpath.");
		}
		long size = ObjectSizer.getObjectSize(theClass);
		System.out.println("Approximate size of " + theClass + " objects :"
				+ size);
	}
